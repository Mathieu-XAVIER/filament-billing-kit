<div>
    ...
    @if($step === 'success')
        <div class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ trans('filament-billing-kit::front/plans.subscription_activated') }}</h2>
            <p class="mt-2 text-gray-500 dark:text-gray-400 max-w-sm">
                {{ trans('filament-billing-kit::front/plans.subscription_activated_message') }}
            </p>
            <button
            ...
            >
                {{ trans('filament-billing-kit::front/plans.view_subscription') }}
            </button>
        </div>
    ...
    @elseif($step === 'canceled')
        <div class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ trans('filament-billing-kit::front/plans.payment_canceled') }}</h2>
            <p class="mt-2 text-gray-500 dark:text-gray-400 max-w-sm">
                {{ trans('filament-billing-kit::front/plans.payment_canceled_message') }}
            </p>
            <button
                wire:click="backToPlans"
                class="mt-8 rounded-lg bg-gray-900 dark:bg-white px-5 py-2.5 text-sm font-semibold text-white dark:text-gray-900 hover:opacity-80 transition-opacity"
            >
                {{ trans('filament-billing-kit::front/plans.back_to_plans') }}
            </button>
        </div>
    ...
    @else
        @if($subscription)
            @php
                ...
                $accentColor = match($subscription->stripe_status) {
                    'active'   => 'border-l-green-500',
                    'trialing' => 'border-l-blue-500',
                    'past_due' => 'border-l-red-500',
                    default    => 'border-l-gray-400',
                };
            @endphp
            <div class="mb-8 rounded-xl border border-gray-200 dark:border-gray-700 border-l-4 {{ $accentColor }} bg-white dark:bg-gray-900 px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-sm">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-0.5">
                            {{ trans('filament-billing-kit::front/plans.current_subscription_label') }}
                        </p>
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            @if($subscription->onGracePeriod())
                                <span class="fbk-status-alert fbk-status-alert--warning py-0.5 px-2.5 text-xs rounded-full">
                                    {{ trans('filament-billing-kit::front/plans.grace_period_ends', ['date' => $subscription->ends_at->format('d/m/Y')]) }}
                                </span>
                            @endif
                        </div>
                    </div>
                ...
                <button
                    ...
                    class="inline-flex items-center gap-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-colors shrink-0"
                >
                    <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M2.5 4A1.5 1.5 0 001 5.5V6h18v-.5A1.5 1.5 0 0017.5 4h-15zM19 8.5H1v6A1.5 1.5 0 002.5 16h15a1.5 1.5 0 001.5-1.5v-6zM3 13.25a.75.75 0 01.75-.75h1.5a.75.75 0 010 1.5h-1.5a.75.75 0 01-.75-.75zm4.75-.75a.75.75 0 000 1.5h3.5a.75.75 0 000-1.5h-3.5z" clip-rule="evenodd"/>
                    </svg>
                    {{ trans('filament-billing-kit::front/plans.manage_billing') }}
        ...
        <div class="text-center mb-8">
            <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white">
                {{ $currentPlan
                    ? trans('filament-billing-kit::front/plans.title_change')
                    : trans('filament-billing-kit::front/plans.title_choose') }}
            </h1>
            <p class="mt-2 text-gray-500 dark:text-gray-400">
                {{ trans('filament-billing-kit::front/plans.subtitle') }}
            </p>
        </div>
        ...
        @if($hasYearlyPrices)
            <div class="fbk-billing-toggle-wrapper">
                <span class="fbk-billing-toggle-label {{ $billingPeriod === 'monthly' ? 'font-semibold text-gray-900 dark:text-white' : '' }}">
                    {{ trans('filament-billing-kit::front/plans.monthly') }}
                </span>
                ...
                <span class="fbk-billing-toggle-label {{ $billingPeriod === 'yearly' ? 'font-semibold text-gray-900 dark:text-white' : '' }}">
                    {{ trans('filament-billing-kit::front/plans.yearly') }}
        ...
        @if($plans->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 text-gray-400">
                ...
                <p class="text-sm">{{ trans('filament-billing-kit::front/plans.empty') }}</p>
            </div>
        @else
                ...
                @foreach($plans as $plan)
                    @php
                        ...
                        $pricePeriod = $isYearly
                            ? trans('filament-billing-kit::front/plans.price_period_yearly')
                            : trans('filament-billing-kit::front/plans.price_period_monthly');
                    @endphp
                    ...
                    <div class="fbk-plan-card {{ $plan->is_featured ? 'fbk-plan-card--featured' : '' }} {{ $isCurrent ? 'fbk-plan-card--current' : '' }}">
                        ...
                        @else
                            <div class="fbk-plan-price">
                                <span class="fbk-plan-price__amount" style="font-size:1.5rem">{{ trans('filament-billing-kit::front/plans.custom_quote') }}</span>
                            </div>
                        @endif
                        ...
                        <div class="fbk-plan-cta">
                            @if($isCurrent)
                                <button disabled class="fbk-plan-cta__button fbk-plan-cta__button--current">
                                    {{ trans('filament-billing-kit::front/plans.current_plan') }}
                                </button>
                            @elseif($plan->is_custom_quote)
                                ...
                                >
                                    <span wire:loading.remove wire:target="subscribe({{ $plan->id }})">{{ trans('filament-billing-kit::front/plans.contact_us') }}</span>
                                    <span wire:loading wire:target="subscribe({{ $plan->id }})" class="inline-flex items-center justify-center gap-2">
                                        <svg class="h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                                        </svg>
                                        {{ trans('filament-billing-kit::front/plans.redirecting') }}
                                    </span>
                                </button>
                            @else
                                <button
                                ...
                                >
                                    <span wire:loading.remove wire:target="subscribe({{ $plan->id }})">
                                        {{ $currentPlan
                                            ? trans('filament-billing-kit::front/plans.change_plan')
                                            : trans('filament-billing-kit::front/plans.get_started') }}
                                    </span>
                                    <span wire:loading wire:target="subscribe({{ $plan->id }})" class="inline-flex items-center justify-center gap-2">
                                        <svg class="h-4 w-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
